...
public class HfBlock<TRecord> : ISerializable where TRecord : IHfRecord, new()
{
    ...
    public void Update(TRecord recordToCompare, TRecord recordToUpdate)
    {
        for (var i = 0; i < ValidRecordsCount; i++)
        {
            if (StoredRecords[i].Equals(recordToCompare))
            {
                StoredRecords[i].Update(recordToUpdate);
                return;
            }
        }
    ...
    public void Delete(TRecord recordToCompare)
    {
        ...
        // Moze byt viacero zaznamov vyhovujucich equals
        for (var i = 0; i < ValidRecordsCount; i++)
        {
            if (StoredRecords[i].Equals(recordToCompare))
            {
                ...
                // Treba zopakovat kontrolu na rovnakom indexe, lebo sme sem presunuli posledny zaznam
                i--;
